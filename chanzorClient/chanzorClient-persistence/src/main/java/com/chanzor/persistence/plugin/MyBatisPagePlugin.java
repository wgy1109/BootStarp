package com.chanzor.persistence.plugin;
...
import com.chanzor.util.ReflectHelper;
...
public class MyBatisPagePlugin implements Interceptor {
		...
		if (invocation.getTarget() instanceof RoutingStatementHandler) {
			...
			if (mappedStatement.getId().matches(".*Page$")) {
				...
				if (parameterObject == null) {
					...
					setParameters(countStatement, mappedStatement, countBS,
							parameterObject);
	...
	private String generatePageSql(String sql, PageInfo page) {
		if (page != null && Tools.notEmpty(dialect)) {
			...
			if ("mysql".equals(dialect)) {
				...
				pageSql.append(currentPage * page.getPageSize());
			}else if("portgresql".equals(dialect)){
				pageSql.append(sql);
				pageSql.append(" limit " + page.getLength() + " offset " + (page.getStart()));
			}
	...
	/**
	 * ��SQL����(?)��ֵ,�ο�org.apache.ibatis.executor.parameter.
