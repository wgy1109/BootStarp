...
public class SimpleMailSender {
	...
	public void sendMail(String toAddress, String title, String Content){
		try {
			...
			mailInfo.setValidate(true);
			mailInfo.setUserName(Const.mailAddress);
			mailInfo.setPassword(Const.mailpassword);
			mailInfo.setFromAddress(Const.mailAddress);
			mailInfo.setToAddress(toAddress);
			mailInfo.setSubject(title);
			mailInfo.setContent(Content);
			sendTextMail(mailInfo);
		} catch (Exception e) {
			logger.error("发送邮件发生错误{}"+e, e);
		}
	}

	public static void main(String[] args) throws Exception {
		// 这个类主要来发送邮件
		String toAddress = "1030485517@qq.com";
		String title = "设置邮箱标题";
		String content = "设置邮箱内容1234567做打算dddddddddddddddddddddddddddddddddddddddddGV速度";
		SimpleMailSender sms = new SimpleMailSender();
		sms.sendMail(toAddress,title,content);
	}
