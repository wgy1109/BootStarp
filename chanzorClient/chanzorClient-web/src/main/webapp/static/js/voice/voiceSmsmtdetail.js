				...
				.dataTable(
						{
							...
							ajax : {
								type : "post",
								url : "voicesmsSendTaskClient/getMtList",
								dataSrc : "data",
								data : data,
											...
											function(i, e) {
												if (i != 0) {
													if(arrayObj[i - 1] == null ){
														$(this).after("<tr><td colspan='5' style='text-align: right;'><a href='javascript:;' onclick='smslistenTest(\"" 
																+ $.trim(arrayfile[i - 1]) 
																+ "\")'><span class='text-danger'>语音试听</span></a></td></tr>")
													}else{
														$(this).after("<tr><td colspan='5' style='text-align: right;'>"
...
function exportPOI() {
	layer.load(2);
	window.location.href = "voicesmsSendTaskClient/exportMtListToExcel?" + $("#form1").serialize();
...
function closelayer(){
	$.ajax({
         type: "post",
         url: "voicesmsSendTaskClient/closetype",
