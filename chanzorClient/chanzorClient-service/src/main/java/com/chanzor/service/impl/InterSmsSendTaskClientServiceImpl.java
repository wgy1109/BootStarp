...
@Service("InterSmsSendTaskClientService")
@SuppressWarnings("unchecked")
public class InterSmsSendTaskClientServiceImpl  implements InterSmsSendTaskClientService{
	...
	public List<Map<String, Object>> getAllSmsSendTaskClientByPage (PageInfo page )throws Exception{
		Map<String, Object> countAll = (Map<String, Object>)daoSupport.findForObject("InterSmsSendTaskClientMapper.getAllSmsSendTaskClientCount", page);
		page.setTotalSize(Integer.parseInt(countAll.get("count").toString()));
		return (List<Map<String, Object>>) daoSupport.findForList("InterSmsSendTaskClientMapper.getAllSmsSendTaskClientP", page);
	}
	...
	public List<Map<String, Object>> getSendStatisticsServiceByPage(PageInfo page) throws Exception {
		...
		return (List<Map<String, Object>>) daoSupport.findForList("InterSmsSendTaskClientMapper.getSendStatisticsByPage", page);
	}
	...
	public String getSql(PageInfo page){
		...
		if( "".equals(formdata.get("timegroup")) && "0".equals(formdata.get("spId"))){
			sql = "select  COALESCE(sum(s.submit_count),0) ALLNUM, COALESCE(sum(s.sended_count),0) SENDEDNUM, COALESCE(sum(s.success_count),0) ALLYES, COALESCE(sum(s.fail_count),0) ALLNO, COALESCE(sum(s.unknown_count),0) OTHERNUM "+
				   "from inter_sms_send_statistics s where 1 = 1 " + stringbuffer.toString();
	...
	public List<Map<String, Object>> getSmsMtListByPage(PageInfo page) throws Exception {
		Map<String, Object> countAll = (Map<String, Object>) daoSupport
				.findForObject("InterSmsSendTaskClientMapper.getSmsMtListClientCount", page);
		page.setTotalSize(Integer.parseInt(countAll.get("count").toString()));
		return (List<Map<String, Object>>) daoSupport.findForList("InterSmsSendTaskClientMapper.getSmsMtListClientP", page);
	}
